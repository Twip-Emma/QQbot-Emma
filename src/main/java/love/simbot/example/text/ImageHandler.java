...
public class ImageHandler {
    ...
    @OnPrivate//复读时，只选择其中的图片发送，否则不发送
    public void RepGetImage(PrivateMsg msg,MsgSender sender){
        MessageContent msgContent = msg.getMsgContent();
        List<Neko> nekoList = msgContent.getCats("image");

        String join = String.join("", nekoList);

        System.out.println(join);

        sender.SENDER.sendPrivateMsg(msg.getAccountInfo().getAccountCode(),join);
    }
}
